        ...
        with tf.Session(config=config) as sess:
            with tf.variable_scope('AST', reuse=tf.AUTO_REUSE):
                ...
                with LocalTFRunner(
                    ...
                    #sim = AST_simulator_wrapper(**sim_args)
                    reward_function = CNReward(**reward_args)
                    spaces = CNSpaces(**spaces_args)
                    ...
                    if save_expert_trajectory:
                        load_convert_and_save_mcts_expert_trajectory(
                            best_actions_filename='home/ubuntu/crowd_nav_ast/best_actions.p',
                            expert_trajectory_filename='home/ubuntu/crowd_nav_ast/expert_trajectories.p',
                            sim=sim,
                            s_0=env_args['s_0'],
                            reward_function=reward_function)
