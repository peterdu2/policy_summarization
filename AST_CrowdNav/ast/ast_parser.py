...
if __name__ == '__main__':
    ...
    for (action_seq, reward_predict) in ast_results:
        ...
        sim.reset(s_0=s_0)
        sim.render(save_render=False, render_path=save_render_path, pause=1000000.)

        done_traj = False
        for action in action_seq:
            sim.step(action.action)
            #print(sim.sim_infos)
            sim.render(save_render=False, render_path=save_render_path, pause=0.)
